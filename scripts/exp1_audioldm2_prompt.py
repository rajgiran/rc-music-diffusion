...
# -*- coding: utf-8 -*-
"""
AudioLDM2 prompt-only generation + comprehensive metrics, visualizations, and HTML report with audio players.
- Works with AudioLDM2 checkpoints (e.g., cvssp/audioldm2, cvssp/audioldm2-large if available).
...
- Metrics:
  ...
  * Audio quality: RMS, crest factor, ZCR, spectral centroid, spectral rolloff, SNR (median-filter approx),
    dynamic range (peak/RMS), duration, RMS dBFS, peak amp
- Visualizations:
...
def main():
    ...
    # generation
    ap.add_argument("--model_id", type=str, default="cvssp/audioldm2",
                    help="e.g., cvssp/audioldm2 (or cvssp/audioldm2-large if available)")
    ...
    # Pipeline
    print(f"[INFO] model={args.model_id}  steps={args.steps}  guidance={args.guidance}")
    ...
    if hasattr(pipe, "safety_checker"):  # disable safety for research
    ...
    for i in range(0, len(prompts), B):
        ...
        negative_list = [args.negative] * len(batch_prompts)  # ✅ match batch size
...
<title>AudioLDM2 Prompt Report</title>
<style>
...
img{max-width:100%}
        ...
        summary_html = f"""
<h1>AudioLDM2 Prompt-Only Report</h1>
